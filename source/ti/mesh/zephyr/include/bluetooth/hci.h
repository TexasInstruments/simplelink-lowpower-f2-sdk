...
ZEPHYR__PACKED(struct) bt_hci_cp_configure_data_path {
...
ZEPHYR__PACKED(struct) bt_hci_rp_configure_data_path {
	uint8_t  status;
};
...
ZEPHYR__PACKED(struct) bt_hci_std_codec_info {
	uint8_t codec_id;
};
ZEPHYR__PACKED(struct) bt_hci_std_codecs {
	uint8_t num_codecs;
	struct bt_hci_std_codec_info codec_info[0];
};
ZEPHYR__PACKED(struct) bt_hci_vs_codec_info {
	uint16_t company_id;
	uint16_t codec_id;
};
ZEPHYR__PACKED(struct) bt_hci_vs_codecs {
	uint8_t num_codecs;
	struct bt_hci_vs_codec_info codec_info[0];
};
ZEPHYR__PACKED(struct) bt_hci_rp_read_codecs {
	uint8_t status;
...
ZEPHYR__PACKED(struct) bt_hci_std_codec_info_v2 {
	uint8_t codec_id;
	uint8_t transports; /* bitmap */
};
ZEPHYR__PACKED(struct) bt_hci_std_codecs_v2 {
	uint8_t num_codecs;
	struct bt_hci_std_codec_info_v2 codec_info[0];
};
ZEPHYR__PACKED(struct) bt_hci_vs_codec_info_v2 {
	uint16_t company_id;
	uint16_t codec_id;
	uint8_t transports; /* bitmap */
};
ZEPHYR__PACKED(struct) bt_hci_vs_codecs_v2 {
	uint8_t num_codecs;
	struct bt_hci_vs_codec_info_v2 codec_info[0];
};
ZEPHYR__PACKED(struct) bt_hci_rp_read_codecs_v2 {
	uint8_t status;
	/* other fields filled in dynamically */
	uint8_t codecs[0];
};

ZEPHYR__PACKED(struct) bt_hci_cp_codec_id {
...
ZEPHYR__PACKED(struct) bt_hci_cp_read_codec_capabilities {
	struct bt_hci_cp_codec_id codec_id;
	uint8_t transport;
	uint8_t direction;
};
ZEPHYR__PACKED(struct) bt_hci_codec_capability_info {
	uint8_t length;
	uint8_t data[0];
};
ZEPHYR__PACKED(struct) bt_hci_rp_read_codec_capabilities {
	uint8_t status;
...
ZEPHYR__PACKED(struct) bt_hci_cp_read_ctlr_delay {
	struct bt_hci_cp_codec_id codec_id;
	uint8_t transport;
	uint8_t direction;
...
ZEPHYR__PACKED(struct) bt_hci_rp_read_ctlr_delay {
	uint8_t status;
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_set_cl_cte_tx_enable {
	uint8_t handle;
	uint8_t cte_enable;
};
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_set_cl_cte_tx_params {
	uint8_t handle;
	uint8_t cte_len;
	uint8_t cte_type;
	uint8_t cte_count;
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_set_conn_cte_tx_params {
	uint16_t handle;
	uint8_t  cte_types;
	uint8_t  switch_pattern_len;
	uint8_t  ant_id[0];
...
ZEPHYR__PACKED(struct) bt_hci_rp_le_set_conn_cte_tx_params {
	uint8_t  status;
	uint16_t handle;
};
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_set_per_adv_recv_enable {
	uint16_t handle;
	uint8_t  enable;
};

#define BT_HCI_OP_LE_PER_ADV_SYNC_TRANSFER      BT_OP(BT_OGF_LE, 0x005a)
ZEPHYR__PACKED(struct) bt_hci_cp_le_per_adv_sync_transfer {
	uint16_t conn_handle;
	uint16_t service_data;
	uint16_t sync_handle;
};

ZEPHYR__PACKED(struct) bt_hci_rp_le_per_adv_sync_transfer {
	uint8_t  status;
	uint16_t conn_handle;
};

#define BT_HCI_OP_LE_PER_ADV_SET_INFO_TRANSFER  BT_OP(BT_OGF_LE, 0x005b)
ZEPHYR__PACKED(struct) bt_hci_cp_le_per_adv_set_info_transfer {
	uint16_t conn_handle;
	uint16_t service_data;
	uint8_t  adv_handle;
};

ZEPHYR__PACKED(struct) bt_hci_rp_le_per_adv_set_info_transfer {
	uint8_t  status;
	uint16_t conn_handle;
};
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_past_param {
	uint16_t conn_handle;
	uint8_t  mode;
	uint16_t skip;
	uint16_t timeout;
...
ZEPHYR__PACKED(struct) bt_hci_rp_le_past_param {
	uint8_t  status;
	uint16_t conn_handle;
};

#define BT_HCI_OP_LE_DEFAULT_PAST_PARAM         BT_OP(BT_OGF_LE, 0x005d)
ZEPHYR__PACKED(struct) bt_hci_cp_le_default_past_param {
	uint8_t  mode;
	uint16_t skip;
	uint16_t timeout;
	uint8_t  cte_type;
...
ZEPHYR__PACKED(struct) bt_hci_rp_le_default_past_param {
	uint8_t  status;
};
...
ZEPHYR__PACKED(struct) bt_hci_cis_params_test {
	uint8_t  cis_id;
	uint8_t  nse;
	uint16_t m_sdu;
	uint16_t s_sdu;
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_set_cig_params_test {
	uint8_t  cig_id;
	uint8_t  m_interval[3];
	uint8_t  s_interval[3];
	uint8_t  m_ft;
...
ZEPHYR__PACKED(struct) bt_hci_rp_le_set_cig_params_test {
	uint8_t  status;
	uint8_t  cig_id;
	uint8_t  num_handles;
	uint16_t handle[0];
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_create_big_test {
	uint8_t  big_handle;
	uint8_t  adv_handle;
	uint8_t  num_bis;
	uint8_t  sdu_interval[3];
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_iso_transmit_test {
	uint16_t handle;
	uint8_t  payload_type;
};

ZEPHYR__PACKED(struct) bt_hci_rp_le_iso_transmit_test {
	uint8_t  status;
	uint16_t handle;
};

#define BT_HCI_OP_LE_ISO_RECEIVE_TEST           BT_OP(BT_OGF_LE, 0x0071)
ZEPHYR__PACKED(struct) bt_hci_cp_le_iso_receive_test {
	uint16_t handle;
	uint8_t  payload_type;
};

ZEPHYR__PACKED(struct) bt_hci_rp_le_iso_receive_test {
	uint8_t  status;
	uint16_t handle;
};

#define BT_HCI_OP_LE_ISO_READ_TEST_COUNTERS     BT_OP(BT_OGF_LE, 0x0072)
ZEPHYR__PACKED(struct) bt_hci_cp_le_read_test_counters {
	uint16_t handle;
};

ZEPHYR__PACKED(struct) bt_hci_rp_le_read_test_counters {
	uint8_t  status;
	uint16_t handle;
...
ZEPHYR__PACKED(struct) bt_hci_cp_le_iso_test_end {
	uint16_t handle;
};

ZEPHYR__PACKED(struct) bt_hci_rp_le_iso_test_end {
	uint8_t  status;
	uint16_t handle;
	uint32_t received_cnt;
	uint32_t missed_cnt;
...
ZEPHYR__PACKED(struct) bt_hci_evt_le_past_received {
	uint8_t      status;
	uint16_t     conn_handle;
	uint16_t     service_data;
	uint16_t     sync_handle;
...
ZEPHYR__PACKED(struct) bt_hci_evt_le_biginfo_adv_report {
	uint16_t sync_handle;
	uint8_t  num_bis;
	uint8_t  nse;
	uint16_t iso_interval;
...
#if 0
/* Following defines are not being used with zephyr code
 * and are re-defining HCI defines (in hci_event.h)
 */
