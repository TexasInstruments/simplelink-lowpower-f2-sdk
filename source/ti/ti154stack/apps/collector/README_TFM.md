...
* [TFM Specifics](#TFMSpecifics)
  * [Loading the Secure Image](#SecureImageLoading)
...
This collector example leverages Trusted Firmware-M (TFM) to provide increased security for applications based on the CC13x4 and CC26x4 family of devices. This partitions memory into distinct memory regions, both secure and non-secure. More information about TFM and TI's implementation can be found under the security section of the ble5stack user guide. 

### <a name="SecureImageLoading"></a>Loading the Secure Image

The example code provided here is meant to be run in non-secure memory. To handle everything in secure memory, you must flash a precompiled secure image first. This can be found at `<SDK_INSTALL_DIR>/tfm_s/build/cc26x4/production_full/Release/outputs/tfm_s.axf` or `<SDK_INSTALL_DIR>/tfm_s/build/cc26x4/production_full/Release/outputs/tfm_s.hex`. Both are valid images for the secure image, just provided in different formats. After loading the secure image to flash, you can load the compiled non-secure example code to flash as normal.
...
This example project implements a collector device: the PAN-Coordinator for the network. This device creates a TI 15.4-Stack network, allows sensor devices to join the network, collects sensor information sent by devices running the sensor example application, and tracks if the devices are on the network or not by periodically sending tracking request messages. Sensors connected to the collector may or may not implement TFM.
