...
#define DUTY_CYCLE_THRESH_LIMITED (UINT32_MAX/1000)
#endif //DUTY_CYCLE_THRESH_LIMITED

#ifndef DUTY_CYCLE_THRESH_CRITICAL
#define DUTY_CYCLE_THRESH_CRITICAL (UINT32_MAX/1000)
#endif //DUTY_CYCLE_THRESH_CRITICAL

#ifndef DUTY_CYCLE_THRESH_REGULATED
#define DUTY_CYCLE_THRESH_REGULATED (UINT32_MAX/1000)
...
static uint32_t phyMultiplier = 1;
...
static void macDutyCycleUpdatePhy(void)
{
    ...
    if((phyRate >= PHY_MODE_FSK_50K)&&(phyRate <= PHY_MODE_FSK_300K))
    {
        phyMultiplier = DUTYCYCLE_USECPERBYTE_50K/(phyModeFSKdataRate[phyRate]/phyModeFSKdataRate[0]);
    }
}
...
void macDutyCycleInit(void)
{
    ...
    //init values for the fake PIB variables which will be updated based on duty cycle selection from sysconfig
