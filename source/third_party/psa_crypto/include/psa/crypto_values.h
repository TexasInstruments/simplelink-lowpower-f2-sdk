...
/**
 * Returns the block size of a SHA2 alg.
 * Returns zero if the alg is not SHA2.
 */
#define PSA_SHA2_BLOCK_SIZE(alg)                                \
    ((alg == PSA_ALG_SHA_224 || alg == PSA_ALG_SHA_256)   ? 64  \
     : (alg == PSA_ALG_SHA_384 || alg == PSA_ALG_SHA_512) ? 128 \
                                                          : 0)

/**
 * Returns the number of bytes of the input length field
 * for a SHA2 alg.
 * Returns zero if the alg is not SHA2.
 */
#define PSA_SHA2_SUSPEND_INPUT_LENGTH_FIELD_LENGTH(alg)        \
    ((alg == PSA_ALG_SHA_224 || alg == PSA_ALG_SHA_256)   ? 8  \
     : (alg == PSA_ALG_SHA_384 || alg == PSA_ALG_SHA_512) ? 16 \
                                                          : 0)

/**
 * Returns the number of bytes of the hash suspend state field
 * for a SHA2 alg.
 * Returns zero if the alg is not SHA2.
 */
#define PSA_SHA2_SUSPEND_HASH_STATE_FIELD_LENGTH(alg)          \
    ((alg == PSA_ALG_SHA_224 || alg == PSA_ALG_SHA_256)   ? 32 \
     : (alg == PSA_ALG_SHA_384 || alg == PSA_ALG_SHA_512) ? 64 \
                                                          : 0)
...
#define PSA_ALG_SIGN_GET_HASH(alg)                                                                                    \
    (PSA_ALG_IS_HASH_AND_SIGN(alg)                                                                                    \
         ? ((alg)&PSA_ALG_HASH_MASK) == 0 ? /*"raw" algorithm*/ 0 : ((alg)&PSA_ALG_HASH_MASK) | PSA_ALG_CATEGORY_HASH \
         : 0)
