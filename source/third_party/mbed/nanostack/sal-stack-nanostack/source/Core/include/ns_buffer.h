...
typedef struct buffer {
    ...
    uint16_t            ipv6_buf_ptr;                /*!< Current pointer in the buffer */
#endif
...
#ifdef WISUN_NCP_ENABLE
/** get Ipv6 data length*/
#define buffer_ipv6_length(x)  (int)(x->buf_end - x->ipv6_buf_ptr)

/** get pointer to ipv6 data*/
#define buffer_ipv6_pointer(x)  (&(x)->buf[(x)->ipv6_buf_ptr])

/** get pointer to ip*/
#define buffer_ip_pointer_set(x,new_start_ptr)  ((x)->ipv6_buf_ptr = (new_start_ptr) - (x)->buf)
