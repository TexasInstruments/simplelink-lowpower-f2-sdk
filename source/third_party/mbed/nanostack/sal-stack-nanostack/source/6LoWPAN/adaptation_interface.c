...
fragmenter_interface_t* global_adaptation_intf_ptr = NULL;
#endif

#ifndef FEATURE_TIMAC_SUPPORT
#define LOWPAN_ACTIVE_UNICAST_ONGOING_MAX 10
#else
#define LOWPAN_ACTIVE_UNICAST_ONGOING_MAX 4
#endif
...
#ifdef FEATURE_TIMAC_SUPPORT
static void lowpan_data_confirm_on_data_req_failure(protocol_interface_info_entry_t *cur, uint8_t msdu_handle, uint8_t status);
#endif
...
#ifdef FEATURE_TIMAC_SUPPORT
/* This API is to be called when TI MAC does not accept packet for transmission */
static void lowpan_data_confirm_on_data_req_failure(protocol_interface_info_entry_t *cur, uint8_t msdu_handle, uint8_t status)
{
    struct mcps_data_conf_s data_cnf;
    data_cnf.msduHandle = msdu_handle;
    data_cnf.status = status;
    data_cnf.timestamp = 0;
    data_cnf.cca_retries = 0;
    data_cnf.tx_retries = 0;
    lowpan_adaptation_interface_tx_confirm(cur, &data_cnf);
}
#endif
...
static void lowpan_data_request_to_mac(protocol_interface_info_entry_t *cur, buffer_t *buf, fragmenter_tx_entry_t *tx_ptr, fragmenter_interface_t *interface_ptr)
{
    ...
    if (interface_ptr->mpx_api) {
        ...
        status = cur->mac_api->mcps_data_req_ext(cur->mac_api, (void *)&dataReq,
                                                            ie_list.payloadIeVectorList, NULL, NULL, NULL);
        if(status)
        {
...
static bool lowpan_buffer_tx_allowed(fragmenter_interface_t *interface_ptr, buffer_t *buf)
{
    ...
    if (is_unicast && interface_ptr->activeTxList_size >= LOWPAN_ACTIVE_UNICAST_ONGOING_MAX) {
        //New TX is not possible there is already too many active connecting
        return false;
    }
...
#else
    if ( ws_info(cur) || (cur->lowpan_info & INTERFACE_NWK_BOOTSRAP_MLE)) {
#endif
