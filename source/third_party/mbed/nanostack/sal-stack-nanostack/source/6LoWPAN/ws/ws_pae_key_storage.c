...
#ifdef NV_RESTORE
    //this change is being done to ensure the file name retains "key_storage_xx" name
    // where xx are ascii chars for digits
    // When file name is being used to derive sub-ID tag for NV storage, the xx from
    // file name is converted to decimal number.
    // example: key_storage_05 will yield a sub-ID of 05
    file_name[12] = (file_number / 10) + '0'; // 0 is ASCII 'A', 1='B',..., F='P'
    file_name[13] = (file_number % 10) + '0'; // same as above
#else
...
static void ws_pae_key_storage_trace(uint16_t field_set, sec_prot_keys_storage_t *key_storage, key_storage_array_t *key_storage_array)
{
    ...
    /* This line seemed to cause crash at random times */
