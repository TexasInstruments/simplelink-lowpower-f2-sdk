...
{
    eapol_relay_t *eapol_relay = ws_eapol_relay_get(interface_ptr);
    tr_extra_debug("ws_eapol_relay_eapol_pdu_receive, eui64: %s", trace_array(kmp_address_eui_64_get((const kmp_addr_t *)eui_64), 8));
    if (!eapol_relay) {
        return -1;
    }
    tr_extra_debug("ws_eapol_relay_eapol_pdu_receive: send to relay, eui64: %s", trace_array(kmp_address_eui_64_get(eui_64), 8));
...
static void ws_eapol_relay_socket_cb(void *cb)
{
    ...
    tr_extra_debug("ws_eapol_relay_socket_cb: enter");

    if (cb_data->event_type != SOCKET_DATA) {
        /* no socket data. Came here for received key req */
        return;
    }
    ...
    if (!eapol_relay) {
        tr_extra_debug("ws_eapol_relay_socket_cb: fail & drop as am not an eapol_relay");
        return;
    }

    uint8_t *socket_pdu = ns_dyn_mem_temporary_alloc(cb_data->d_len);
    if (!socket_pdu) {
        tr_extra_debug("ws_eapol_relay_socket_cb: memory alloc failed");
        return;
    }
    ...
    if (socket_recvfrom(cb_data->socket_id, socket_pdu, cb_data->d_len, 0, &src_addr) != cb_data->d_len) {
        tr_extra_debug("ws_eapol_relay_socket_cb: fail Freeing packet as we did not receive of len %d", cb_data->d_len);
    ...
    if (cb_data->d_len == 9) {
        tr_extra_debug("ws_eapol_relay_socket_cb: fail - insufficient length");
        ws_eapol_pdu_mpx_eui64_purge(eapol_relay->interface_ptr, socket_pdu);
        ns_dyn_mem_free(socket_pdu);
        return;
    }

    tr_extra_debug("ws_eapol_relay_socket_cb: send to mpx");
    ...
    if (ws_eapol_pdu_send_to_mpx(eapol_relay->interface_ptr, socket_pdu, socket_pdu + 8, cb_data->d_len - 8, socket_pdu, NULL, 0) < 0) {
        tr_extra_debug("ws_eapol_relay_socket_cb: send to mpx failed");
        ns_dyn_mem_free(socket_pdu);
    }
}
