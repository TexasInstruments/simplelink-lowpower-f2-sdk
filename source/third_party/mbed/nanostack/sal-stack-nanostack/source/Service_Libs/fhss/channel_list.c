...
const int CHANNEL_LIST_SIZE_IN_BITS = 129;
...
static bool channel_list_bit_test(const uint8_t *list, int bit_number)
{
    const int_fast8_t word_index = bit_number / 8;
    const int_fast8_t bit_index = bit_number % 8;

    return channel_list_bit_test32(list[word_index], bit_index);
}

static uint8_t channel_list_search(const uint8_t *list, int index)
{
    uint8_t channel = 0;
...
uint8_t channel_list_get_channel(const uint8_t *list, int current_index)
{
    uint8_t found_index;
...
void channel_list_set_channel(uint8_t *list, int channel, bool active)
{
    if (channel >= CHANNEL_LIST_SIZE_IN_BITS) {
        return;
    }
    if (active) {
        list[channel / 8] |= (1 << channel % 8);
    } else {
        list[channel / 8] &= ~(1 << channel % 8);
    }
    return;
