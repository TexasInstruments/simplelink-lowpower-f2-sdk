...
/* TI Wi-SUN FAN Stack External Debug Variabl */
#ifdef DBG_WISUN
typedef enum fwh {
    ERROR_NONE,
    PTK_KEY_GEN_ERR_MSG1,
    PTK_KEY_GEN_ERR_MSG3,
    ERR_MSG3
} FWH_STATE;
...
struct wisun_debug {
    uint16_t req[7];
    uint16_t ind[7];
    uint16_t mem_alloc_failures;
    uint16_t temp_mem_alloc_failures;
    uint32_t mem_bytes_allocated;
    uint32_t max_mem_bytes_allocated;
    uint16_t dao_create_cnt;
    uint16_t dao_deleted_cnt;
    uint16_t dao_list_size;
    FWH_STATE fwh_error;
};
