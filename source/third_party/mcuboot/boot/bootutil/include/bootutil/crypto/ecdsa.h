...
#if defined(MCUBOOT_USE_CC310) || defined(MCUBOOT_USE_MBED_TLS) || defined(MCUBOOT_USE_TI_CRYPTO)
...
#if defined(MCUBOOT_USE_TINYCRYPT) || defined(MCUBOOT_USE_MBED_TLS) || defined(MCUBOOT_USE_CC310) || defined(MCUBOOT_USE_TI_CRYPTO) && !defined(MCUBOOT_USE_PSA_CRYPTO)
/*
...
#endif /* (MCUBOOT_USE_TINYCRYPT || MCUBOOT_USE_MBED_TLS || MCUBOOT_USE_CC310) || defined(MCUBOOT_USE_TI_CRYPTO) && !MCUBOOT_USE_PSA_CRYPTO */
...
#if defined(MCUBOOT_USE_TI_CRYPTO)
typedef void* bootutil_ecdsa_context;
/*
 * Read in signature. Signature has r and s encoded as integers. Helper function.
 */
static int bootutil_read_bigint(uint8_t i[NUM_ECC_BYTES], uint8_t **cp, uint8_t *end)
                                        ...
                                        uint8_t *sig, size_t sig_len)
{
    ...
    return SlCrypto_ecdsa_p256_verify(pk, hash, signature);
}
static inline int bootutil_ecdsa_parse_public_key(bootutil_ecdsa_context *ctx,
                                                  uint8_t **cp,uint8_t *end)
{
