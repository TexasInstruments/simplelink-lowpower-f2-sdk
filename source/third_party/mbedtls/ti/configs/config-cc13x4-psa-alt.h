/**
 * \file config-cc13x4-psa-alt.h
 *
 * \brief Configuration options (set of defines) for mbedTLS with 
 *        PSA Crypto API enabled with ALT layer implementation
 *        using TI SL Drivers. 
 *
...
/* TI-MBEDTLS: Size of static buffer used for allocating memory
 *
 * The buffer size is allocated based on assumption of 16 largest symmetric keys (32B) and 16 largest asymmetric
 * public keys (133B) that can be supported by KeyStore, with surplus bytes for
 * additional calloc calls within mbedTLS.
 */
#define MBEDTLS_STATIC_MEMORY_BUFFER_ALLOC_SIZE 3072

/**
 * TI-MBEDTLS: This is custom define that disables certain features
