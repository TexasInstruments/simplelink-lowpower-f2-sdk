    ...
    mbedtls_ecp_keypair **p_ecp)
{
    ...
    if (PSA_KEY_TYPE_IS_PUBLIC_KEY(type) &&
         ...
         */
         /* TI-MBEDTLS: Changes to support LE Weierstrass curves */
    ...
    else if (PSA_KEY_TYPE_ECC_GET_FAMILY(type) == PSA_ECC_FAMILY_MONTGOMERY)
    {
        /* TI-MBEDTLS: Changes to support BE Curve25519 keys for TI SL Keystore.
         *             BE Curve25519 public keys are represented using
         *             using 65B - [0x04 | x-coordinate | y-coordinate], which is
         *             different from LE Curve25519 public keys which are 
         *             represented using 32B - [x-coordinate].
         */
        if (curve_bytes == 65)
        {
            curve_bytes = data_length / 2;
    ...
    if (PSA_KEY_TYPE_IS_PUBLIC_KEY(type)) {
        ...
        /* TI-MBEDTLS: TI Driver performs checks on points before using them */
