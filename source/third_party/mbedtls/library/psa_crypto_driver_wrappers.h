...
#endif
/* Headers for ti_sl transparent driver */
#if defined(ENABLE_TI_SL_MBEDTLS_PSA_DRIVER)
#include "ti_sl_transparent_driver_entrypoints.h"
...
#define TI_SL_TRANSPARENT_DRIVER_ID (5)
...
#if defined(PSA_TI_SL_BUILT_IN_KEY_SUPPORT)
    status = ti_sl_transparent_builtin_key_loader_init();
    if(status != PSA_SUCCESS)
        return( status );
#endif
...
#if (defined(ENABLE_TI_SL_MBEDTLS_PSA_DRIVER) )
            status = ti_sl_transparent_import_key
                (attributes,
                                data,
                                data_length,
                                key_buffer,
