...
#if TFM_ENABLED
/*
 * [TI-TFM] Set to arbitrary non-zero handle to avoid error since key init is
 * skipped because attestation keys are hard-coded into psa_sign_hash and
 * psa_update_hash.
 */
static psa_key_handle_t attestation_key_handle = 1;
#else
...
#if TFM_ENABLED
/*
 * [TI-TFM] Set to valid length to avoid error return since key init is
 * skipped because attestation keys are hard-coded into psa_sign_hash and
 * psa_update_hash.
 */
static size_t   attestation_public_key_len = ECC_P256_PUBLIC_KEY_SIZE;
#else
