...
/*
 * [TI-TFM] Skip key registration. CryptoKeyKeyStore_PSA functions are not
 * currently callable from secure partitions outside the Crypto partition.
 * Therefore, the attestation private key is hard-coded into ti_psa_sign_hash()
 * and the public key is hard-coded into ti_psa_hash_update().
 */
#if (TFM_ENABLED == 0)
...
/* [TI-TFM] Reading the public key is not supported */
#if defined(SYMMETRIC_INITIAL_ATTESTATION) || defined(TFM_ENABLED)
psa_status_t
