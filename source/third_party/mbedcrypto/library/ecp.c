                                    ...
                                    unsigned char *buf, size_t buflen )
{
    ...
    {
        /* TI-MBEDCRYPTO: Changed to support big-endian montgomery keys */
        if (P->Y.n != 0x00)
        {
            *olen = 2*plen + 1;
            if( buflen < *olen )
                                   ...
                                   const unsigned char *buf, size_t ilen )
{
    ...
    if( mbedtls_ecp_get_type( grp ) == MBEDTLS_ECP_TYPE_MONTGOMERY )
    {
        ...
        /* TI-MBEDCRYPTO: Disabled this check as it is taken care by TI drivers */
    ...
    if( mbedtls_ecp_get_type( grp ) == MBEDTLS_ECP_TYPE_SHORT_WEIERSTRASS )
    {
        ...
        /* TI-MBEDCRYPTO: Changed to support big-endian and little-endian weierstrass keys */
        if( buf[0] != 0x04 && (ilen == 2 * plen) )
        {
            MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &pt->X, buf, plen ) );
            MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &pt->Y, buf + plen, plen ) );
        }
        else if ( (buf[0] == 0x04) && (ilen == 2 * plen + 1) )
        {
            MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary( &pt->X, buf + 1, plen ) );
