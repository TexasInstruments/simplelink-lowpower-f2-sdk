...
/* 
 * TI-MBEDCRYPTO:mbedtls_ecc_group_to_psa only used by ECDH,
 * restricting to reduce code size(psa_key_agreement_ecdh)
 */
                                              ...
                                              mbedtls_ecp_keypair **p_ecp )
{
    ...
    /*
     * TI-MBEDCRYPTO: Commented out Public key check because
     * TI drivers public key format is different from mbedcrypto's
     * So, size check will fail. Also, the public key properties 
     * are checked by TI drivers before usage.
     */
    // status = mbedtls_to_psa_error(
                                               ...
                                               mbedtls_ecp_keypair **p_ecp )
{
    ...
    /* 
     * TI-MBEDCRYPTO: Commented private key validation while storing private key 
     * as private keys are already validated by TI Drivers before their use 
     */
    // status = mbedtls_to_psa_error(
    ...
    /* 
     * TI-MBEDCRYPTO: Commented public key generation while storing private key.
     * This point multiplication is performed in software, which is slow.
     * And, TI Driver computes public key when needed from private key
     */
    // status = mbedtls_to_psa_error(
...
/* TI-MBEDCRYPTO: Exposed this function to help obtain plaintext keys on the secure side */
psa_status_t psa_get_key_from_slot( psa_key_handle_t handle,
...
/* TI-MBEDCRYPTO: Exposed this function to obtain plaintext keys on the secure side */
...
psa_status_t psa_mac_abort( psa_mac_operation_t *operation )
{
    ...
    if( operation->alg == PSA_ALG_CMAC )
    {
        ...
        /* 
         * TI-MBEDCRYPTO: Moved resetting structure to conditional 
         * statements to avoid IAR warnings
         */
        operation->alg = 0;
        operation->key_set = 0;
        operation->iv_set = 0;
    ...
    if( PSA_ALG_IS_HMAC( operation->alg ) )
    {
        ...
        /* 
         * TI-MBEDCRYPTO: Moved resetting structure to conditional 
         * statements to avoid IAR warning
         */
        operation->alg = 0;
        operation->key_set = 0;
        operation->iv_set = 0;
    ...
    /* 
     * TI-MBEDCRYPTO: Moved resetting structure to conditional 
     * statements to avoid IAR build warning
     */
    // operation->alg = 0;
    // operation->key_set = 0;
    // operation->iv_set = 0;
                             ...
                             size_t input_length )
{
 ...
 /* TI-MBEDCRYPTO: Dummy line added to avoid unused variable build warning */
#if !defined(MBEDTLS_CMAC_C) && !defined(MBEDTLS_MD_C)
    (void)input;
    (void)input_length;
#endif
    ...
    if( operation->alg == PSA_ALG_CMAC )
    {
        ...
        /* 
         * TI-MBEDCRYPTO: Moved function exit to conditional 
         * statements to avoid IAR build warning
         */
        if( status != PSA_SUCCESS )
            psa_mac_abort( operation );
...
{
/* TI-MBEDCRYPTO: Dummy line added to avoid unused variable build warning */
#if !defined(MBEDTLS_PSA_CRYPTO_SE_C) && !defined(MBEDTLS_RSA_C) && !defined(MBEDTLS_ECDSA_C)
    (void)hash;
    (void)hash_length;
#endif
    psa_key_slot_t *slot;
...
typedef struct
{
    psa_key_slot_t *slot;
    const mbedtls_cipher_info_t *cipher_info;
    /* TI-MBEDCRYPTO: Restricting the union based on defined crypto algorithm */
#if defined(MBEDTLS_CCM_C) || defined(MBEDTLS_GCM_C) || defined(MBEDTLS_CHACHAPOLY_C)
    union
    {
                                    ...
                                    psa_algorithm_t alg )
{
    ...
    switch( PSA_ALG_AEAD_WITH_TAG_LENGTH( alg, 0 ) )
    {
        ...
        case PSA_ALG_AEAD_WITH_TAG_LENGTH( PSA_ALG_CCM, 0 ):
            ...
            /* TI-MBEDCRYPTO: Moved error check in switch case to prevent IAR build warning */
                               ...
                               size_t *ciphertext_length )
{
    ...
    *ciphertext_length = 0;
    ...
    if( operation.core_alg == PSA_ALG_GCM )
    {
        ...
        /* TI-MBEDCRYPTO: Moved error checking inside switch case to prevent IAR build warning */
...
{
    psa_status_t status;
    /* TI-MBEDCRYPTO: mbedcrypto drbg is not used for RNG by TI drivers */
