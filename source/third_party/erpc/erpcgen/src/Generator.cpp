...
void Generator::makeIncludesTemplateData()
{
    ...
    if (m_def->hasProgramSymbol())
    {
        for (auto include : getAnnotations(m_def->getProgramSymbol(), INCLUDE_ANNOTATION))
        {
            ...
            Log::info("Program level include: include %s\n", include->getValueObject()->toString().c_str());
        }
    }
...
data_list Generator::makeGroupIncludesTemplateData(Group *group)
{
    ...
    for (Interface *iface : group->getInterfaces())
    {
        vector<Annotation *> includeAnnList = iface->getAnnotations(INCLUDE_ANNOTATION, getAnnotationLang());
        if (0 < includeAnnList.size())
        {
            for (int i = 0; i < includeAnnList.size(); ++i)
            {
                string include = includeAnnList[i]->getValueObject()->toString();
                if (tempSet.find(include) == tempSet.end())
                {
                    includes.push_back(include);
                    Log::info("Interface level include: include %s\n", include.c_str());
        ...
        {
            vector<Annotation *> funcAnnList = func->getAnnotations(INCLUDE_ANNOTATION, getAnnotationLang());
            if (0 < funcAnnList.size())
            {
                for (int i = 0; i < funcAnnList.size(); ++i)
                {
                    string include = funcAnnList[i]->getValueObject()->toString();
                    if (tempSet.find(include) == tempSet.end())
                    {
                       includes.push_back(include);
                       Log::info("Function level include: include %s\n", include.c_str());
                       tempSet.insert(include);
                    }
                }
            }
