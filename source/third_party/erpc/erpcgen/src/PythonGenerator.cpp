...
string PythonGenerator::getFunctionPrototype(Function *fn)
{
    ...
    if (params.size())
    {
        ...
        {
            // Skip data serialization for variables placed as @length value for lists,
            // but only if direction is the same, or ref=inout and it=out.
            //
            StructMember *referencedFrom = findParamReferencedFromAnn(params, getOutputName(it), LENGTH_ANNOTATION);
            if (referencedFrom && ((referencedFrom->getDirection() == it->getDirection()) ||
                ((referencedFrom->getDirection() == kInoutDirection) && (it->getDirection() == kOutDirection))))
            {
                continue;
            }
...
void PythonGenerator::setOneStructMemberTemplateData(StructMember *member, data_map &member_info)
{
    ...
    bool isNullable = ((findAnnotation(member, NULLABLE_ANNOTATION) != nullptr) &&
                       (member->isByref() || trueDataType->isFunction() ||
